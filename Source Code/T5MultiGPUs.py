...
# model = T5ForConditionalGeneration.from_pretrained("./savePretrainedT5Small6-25")
...
for epoch in range(epoches):
    ...
    for idx in range(len(trainQC)):
        input_ids = tokenizer(trainQC[idx], add_special_tokens=True, return_tensors="pt", truncation=True,
                              max_length=max_source_length).input_ids.to(device)
        labels = tokenizer(trainAnswer[idx], return_tensors="pt", add_special_tokens=True, truncation=True,
                           max_length=max_target_length).input_ids.to(device)
        if idx % 1000 == 0:
            print("Sample context {}: \n".format(sampleIdx + 1), trainContext[idx])
            print("Sample output {}: \n".format(sampleIdx + 1), tokenizer.decode(
                (model.module.generate(input_ids=input_ids, decoder_start_token_id=0, max_length=max_target_length))[0],
    ...
    with torch.no_grad():
        for idx in range(len(valQC)):
            input_ids = tokenizer(valQC[idx], add_special_tokens=True, return_tensors="pt", truncation=True,
                                  max_length=max_source_length).input_ids.to(device)
            labels = tokenizer(valAnswer[idx], return_tensors="pt", add_special_tokens=True, truncation=True,
                               max_length=max_target_length).input_ids.to(device)
            if idx % 100 == 0:
                print("Sample Val context {}: \n".format(sampleIdx + 1), valContext[idx])
                print("Sample Val output {}: \n".format(sampleIdx + 1), tokenizer.decode((model.module.generate(
                    input_ids=input_ids, decoder_start_token_id=0, max_length=max_target_length))[0]))
    ...
    if epoch + 1 == 1:
        model.module.save_pretrained("./Try")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 6:
        model.module.save_pretrained("./savePretrainedT5Small8-6")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 8:
        model.module.save_pretrained("./savePretrainedT5Small8-8")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 10:
        model.module.save_pretrained("./savePretrainedT5Small8-10")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 12:
        model.module.save_pretrained("./savePretrainedT5Small8-12")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 14:
        model.module.save_pretrained("./savePretrainedT5Small8-14")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 16:
        model.module.save_pretrained("./savePretrainedT5Small8-16")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 18:
        model.module.save_pretrained("./savePretrainedT5Small8-18")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 20:
        model.module.save_pretrained("./savePretrainedT5Small8-20")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 25:
        model.module.save_pretrained("./savePretrainedT5Small8-25")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    if epoch + 1 == 30:
        model.module.save_pretrained("./savePretrainedT5Small8-30")
        bestLoss = totalLoss
        bestValLoss = totalLossVal
        print("Model Saved.")
    print("--------------------------------------------------------------------------------------------------------")
